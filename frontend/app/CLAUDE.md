...
- `src/router/AppRouter.tsx` is the single routing surface (`react-router-dom`). Routes for `/giris` (login) and `/sifremi-unuttum` (password reset) don't render React — they redirect (`window.location.href`) straight to the BFF's Keycloak OAuth2 authorization endpoint (see `auth.api.ts`). Actual login UI is server-rendered by Keycloak using the theme in `src/keycloak-theme/`, not by this router.
